...
Event sizes can be estimated from "first principles" under the assumption of some event topology and noting the significant amount of cosmic contamination at sea level.  As was shown in Sec.~\ref{calibration}, there is expected to be on average $\sim$68 track segments on top of the actual beam event.  Given that a 4 GeV mip will produce on average 80~kbyte of data, the resulting number of particles in a readout window will result in the readout of 6~MB per beam event.  
...
In addition to the data associated with the individual particles in the event, there is channel overhead information.   Experience from the 35 t detector at Fermilab indicates that with zero suppression that there is an event overhead of 6~kB per channel for a readout of three drift windows.
%
...
%
Since there are 15,360 channels in the CERN prototype, the expected overhead corresponds to 92~MB per event.  The resulting total event size from overhead and charge from individual particles is 100~MB. 
...
that have and continue to be developed for DUNE, the 35 t detector test and the short baseline program at Fermilab as well as the 
