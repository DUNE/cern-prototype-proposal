...
In addition to the data associated with the individual particles in the event, there is channel overhead information.   Experience from the 35t detector at Fermilab indicates that with zero suppression that there is an event overhead 0.3~kB per channel for a readout of three drift windows.
%
...
%
Since there are 15,360 channels in the CERN prototype, the expected overhead corresponds to 5~MB per event.  Resulting total event size from overhead and charge from individual particles is 11~MB. 
...
Total particle statistics resulting from the number of spills presented in the run plan table~\ref{tab:RunPlan} multiple by data taken at different beam angles is approximately 50M events total.  Taking into account the 11~MB data load per event, this leads to the estimate of 550 TB of nominal data volume to be collected in this experiment. 
