...
The ICARUS T600 detector~\cite{ICARUS1,ICARUS2} pioneered the first large-scale detector operating in the CNGS 
neutrino beam at mean energy $\sim$17 GeV. ArgoNeuT~\cite{argoneut1,argoneut2} recently studied 
...
3~GeV/c (right) incident momenta simulated using FLUKA particle transport code~\cite{fluka05}. Shower reconstruction 
is performed with ICARUS spatial and calorimetric reconstruction algorithm~\cite{icarus_reco}.
65\% of protons with 1~GeV/c will interact while the remaining 
35\% will range out and be used to study particle identification algorithms (PID) (see Sec.~\ref{detbeam_pid}). 
...
for interacting pions ranging from 0.2~GeV/c up to 7~GeV/c momenta simulated using FLUKA~\cite{fluka05}.
...
Furthermore, recombination could introduce a component to the correction that
is currently not modeled. 
Jaffe columnar recombination model~\cite{jaffe,eqref} predicts 
...
%
The Modified Box model~\cite{box} (current default for ICARUS and LArSoft) does not include an
...
track range for muons, pions, protons and kaons for particles simulated using ICARUS MC ???.
...
\label{fig:resrange}
  \caption{de/dx vs. residual range. PID algorithm. 
...
%\includegraphics[width=0.49\textwidth,height=5.0cm]{figures/pid}
%\includegraphics[width=0.49\textwidth,height=5.0cm]{figures/pid_probabilities}
\includegraphics[width=\textwidth,height=10.0cm]{figures/eff-bgdrej-diffen}
\label{fig:egam}
  \caption{Background rejection versus signal efficiency for various energies.
{\color{red}
}
}
\end{figure}
