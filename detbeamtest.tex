...
that are needed to control and understand systematic uncertainties in DUNE oscillation measurements.
...
uncertainty of 10\% shift of lepton enerfy scale on the measured 
appearance signal for $\delta_{CP}=0$ 
and backgrounds in DUNE. The curve shows the expected signal shape for 
$\delta_{CP}=?$. Energy scale uncertainties can mimic a non-zero
$\delta_{CP}$ phase. 
\begin{figure}[h!]
\centering
...
(right) to neutrino energy scale uncertainties. Assumptions are described in the text. 
...
Work to evaluate effect of all systematic uncertaintes in DUNE sensitivities is still in progress. Current levels of sensitivity in ~\cite{DuneCDR} assumes
$\nu_e$ energy scale uncertainties at the level of 2\%. 
More here...
...
Reconstruction of neutrino energy depends of a quality of reconstruction of both electromagnetic and hadronic showers. Accurate
reconstruction of hadron energy 
...
A test beam with know incoming particle type and energy is the ideal case
...
outgoing particles, especially at low energies) must be checked and benchmarked against calibration data to avoid
...
by $\pi^+$ versus $\pi^-$ initiated interactions are different up to energy on the order of 1~GeV due to different
final state particles and interaction cross sections. This is illustrated in 
Fig.~\ref{pionshwr} which shows the differences in mean energy deposited (left) and width (right) 
...
accurately identify particle type as well as measure their kinetic energy. 
Fig.~\ref{fig:resrange} shows track energy loss per unit length (dE/dx) as a function of residual 
track range for muons, pions, protons and kaons. ALSO include the plot of probabilities from Robert here.
